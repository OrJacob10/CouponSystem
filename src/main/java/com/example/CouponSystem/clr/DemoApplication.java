...
public class DemoApplication implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        for (int i = 1; i <= 100; i++) {
            Coupon coupon = Coupon.builder().title("coupon" + i).description("description" + i).category(this.categoryService.getCategory(random.nextInt(1,11))).startDate(LocalDate.now()).endDate(LocalDate.now().plusDays(100 + i)).amount(i*5).price(1).image("image" + i).build();
